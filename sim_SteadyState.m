...
simulation.veh0.x_init     = [0; leader.Vdes0; 0];
simulation.veh1.x_init     = [-(control.ri + control.h*leader.Vdes0 + control.L); leader.Vdes0; 0];
mode0 = 0;
...
axes('position',[.35 .5 .2 .1])
box on % put box around new pair of axes
indexOfInterest = (out.tout < 2 & out.tout > 1); % range of t near perturbation
...
plot(out.t_hybrid, out.mode, 'color', colorblind(2,:), 'linewidth', 1.5); hold on;
plot(out.t_hybrid, q_true, 'color', colorblind(1,:), 'linewidth', 1.5); 
set(gca, 'YTick', [0, 1, 2, 3, 4], 'YTickLabel', {'$q_{0}$','$q_{1|1}$', '$q_{1|2}$', '$q_{2|1}$', '$q_{2|2}$', ''}); ylim([0 4.2]); %xlim([0 out.tout(end)])
xlabel('Time [s]', 'fontsize', FontSizeAxes); ylabel('','fontsize', FontSizeAxes)
legend( 'Mode $q_{j|k}$','Ground truth', 'fontsize', FontSizeLegend)
